...
def make_consensus_seq(algn, seq1: SeqRecord, seq2: SeqRecord):
    ...
    consensus_record: SeqRecord = SeqRecord(Seq(''.join(consensus)),
                                                ...
                                                description=f"Consensus sequence generated by GlycoTools")
...
def parse_ab1_v2(filename1, filename2, direction="ff"):
    ...
    if direction == "fr":
        seq2 = seq2.reverse_complement(id=f"revCompl_{seq2.id}")
    return seq1, seq2
...
layout = dbc.Container([
    ...
    html.Div([
        ...
        html.P([
            "Hint: if you want to manually check the alignment:\n"
            " 1. Copy alignment from 'consensus_overview.xlsx' into WordPad.\n"
            " 2. Change font to 'courier'\n"
            " 3. If necessary: go to tab 'View' -> 'Word wrap' -> 'No wrap'"
        ],
        style={"whiteSpace": "pre"})
    ]),
...
def trim_and_consensus(n_clicks, direction, seq1filenames, seq1filedatas, seq2filenames, seq2filedatas):
    ...
    with open(f"consensus_seqs.fasta", "w") as f:
        for basename in seq1_files.keys():
            ...
            consensus.append([seq1_files[basename],
                                ...
                                pairwise2.format_alignment(*alignment[0], full_sequences=False),
    ...
    df = pd.DataFrame(consensus,
                      columns=["Seq1", "Seq2", "Warnings", "Consensus", "Alignment-short", "Alignment-long"])
    in_memory_excel = BytesIO()
    df.to_excel(in_memory_excel, index=False)
    empty_tmpFiles(session_id)

    return [dcc.send_data_frame(df.to_excel, "consensus_overview.xlsx", index=False, sheet_name="analysis"),
